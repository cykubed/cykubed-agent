...
PREFER_READ_ONLY_MANY_ARGS=""


while getopts "ern:t:" opt; do
  case $opt in
     e)
       PREFER_READ_ONLY_MANY_ARGS=" --set preferReadOnlyMany=false"
       ;;
     r)
       REPLICATION_ARGS=""
       ;;
...
helm upgrade --install $NAMESPACE -n $NAMESPACE --create-namespace --set token="$TOKEN" --set tag="$TAG" --set apiUrl="https://dev.cykubed.com/api" --force --set platform=minikube --set imagePullPolicy=IfNotPresent $REPLICATION_ARGS $PREFER_READ_ONLY_MANY_ARGS /tmp/agent-dist/agent-$TAG.tgz
