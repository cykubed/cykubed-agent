...
helm upgrade --install cykubed -n cykubed --create-namespace --set token="$TOKEN" --set tag="$TAG" --set apiUrl="https://dev.cykubed.com/api"  --set platform=minikube --set imagePullPolicy=IfNotPresent --set cache.storageClass=standard --set redis.sentinel.enabled=false --set redis.architecture=standalone  ./chart
